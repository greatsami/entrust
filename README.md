# ENTRUST (Laravel 6 Package) - Copied & Edited from (Zizaco/Entrust)
...
```php
    'role' => \Greatsami\Entrust\Middleware\EntrustRole::class,
    'permission' => \Greatsami\Entrust\Middleware\EntrustPermission::class,
    'ability' => \Greatsami\Entrust\Middleware\EntrustAbility::class,
```
...
use Greatsami\Entrust\EntrustPermission;
...
If you don't, then try `php artisan vendor:publish` and, if it does not appear, manually copy the `/vendor/Greatsami/entrust/src/config/config.php` file in your config directory and rename it `entrust.php`.
